## RRCA12p_08_CombineMODFLOW-AnalyticalOnly.R
# Makes and saves tables combining MODFLOW and analytical model (not ADF) output.
#  - MODFLOW output is from RRCA12p_05_MODFLOW-SummarizeLeakage.R
#  - Analytical model output is from RRCA12p_07a_AnalyticalOnly-CalculateDepletion.R

source(file.path("src", "paths+packages.R"))

## some parameters controlling which analytical results to use
...
ADF_df <- 
  paste0("RRCA12p_07a_ADF-CalculationDepletion_", analytical_model, "_", storage, "_AnalyticalOnly_", paste(str_BCs, collapse = "-"), ".csv") %>% 
...
## save output
capture_all %>% 
  readr::write_csv(file.path("results", paste0("RRCA12p_08_CombineMODFLOW-AnalyticalOnly_Capture_", analytical_model, "_", storage, "_", paste(str_BCs, collapse = "-"), ".csv")))

depletion_all %>% 
  readr::write_csv(file.path("results", paste0("RRCA12p_08_CombineMODFLOW-AnalyticalOnly_Depletion_", analytical_model, "_", storage, "_", paste(str_BCs, collapse = "-"), ".csv")))

mostAffected_all %>% 
  readr::write_csv(file.path("results", paste0("RRCA12p_08_CombineMODFLOW-AnalyticalOnly_MostAffected_", analytical_model, "_", storage, "_", paste(str_BCs, collapse = "-"), ".csv")))
